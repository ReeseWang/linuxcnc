/*
  ...
  userspace HAL program to control a Delta VFD-M VFD

  Yishin Li, adapted from Michael Haberler's vfs11_vfd/.
  Ruoxi Wang, adapted from vfdb_vfc/.
  ...
  see 'man vfdm_vfd' and the VFD-M section in the Drivers manual.
...
// command registers for DELTA VFD-M Inverter
...
#define REG_UPPERLIMIT                  0x0003  // limit on output frequency in VFD
...
// status registers for DELTA VFD-M Inverter
...
#define SR_MOTOR_SPEED_L        0x210B          // RPM
#define SR_MOTOR_SPEED_H        0x210C          // RPM
//#define SR_TORQUE_RATIO         0x2104          // % TODO: there's no torque ration estimation in vfd-m
#define SR_OUTPUT_CURRENT       0x2104          // output curr
#define SR_OUTPUT_VOLTAGE       0x2106          // %
#define SR_INVERTER_MODEL	0x0050
#define SR_RATED_CURRENT	0x0039		// 0.1A
#define SR_RATED_VOLTAGE	0x0005		// 0.1V
#define SR_EEPROM_VERSION	0x0064
 ...
 * operation.
 *
 * However, the interesting VFD-M registers are not contiguous, and must be read
...
//TODO: implement RESET command for VFD-M
...
int read_data(modbus_t *ctx, haldata_t *haldata, param_pointer p)
{
    ...
    *(haldata->freq_out) = freq_reg * 0.01;
    ...
    {
	*(haldata->RPM) = freq_reg * 0.01 / haldata->motor_nameplate_hz \
			  * haldata->motor_nameplate_RPM;
	*(haldata->RPS) = freq_reg * 0.01 / haldata->motor_nameplate_hz \
			  * haldata->motor_nameplate_RPM * 0.01666666667;
	float speed_error;
